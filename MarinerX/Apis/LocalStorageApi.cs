...
namespace MarinerX.Apis
{
    internal class LocalStorageApi
    {
        ...
        public static List<Quote> GetQuotesForOneDay(string symbol, DateTime startTime)
        {
            try
            {
                var data = File.ReadAllLines(PathUtil.BinanceFuturesData.Down("1m", symbol, $"{symbol}_{startTime:yyyy-MM-dd}.csv"));
        ...
        public static Dictionary<string, List<Quote>> GetAllOneDayQuotes()
        {
            try
            {
                ...
                var fileNames = Directory.GetFiles(PathUtil.BinanceFuturesData.Down("1D"), "*.csv");
