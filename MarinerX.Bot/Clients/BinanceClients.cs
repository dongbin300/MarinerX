...
namespace MarinerX.Bot.Clients
{
    public class BinanceClients
    {
        ...
        {
            return await Api.UsdFuturesApi.Trading.PlaceOrderAsync(symbol, OrderSide.Buy, FuturesOrderType.Limit, quantity, price, PositionSide.Long).ConfigureAwait(false);
        }

        public static async Task<WebCallResult<BinanceFuturesPlacedOrder>> OpenSell(string symbol, decimal price, decimal quantity)
        {
            return await Api.UsdFuturesApi.Trading.PlaceOrderAsync(symbol, OrderSide.Sell, FuturesOrderType.Limit, quantity, price, PositionSide.Short).ConfigureAwait(false);
        }
